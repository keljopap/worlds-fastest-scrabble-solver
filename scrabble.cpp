...
void init_tile_bag() {
    tile_bag      = {{'a', 9}, {'b', 2}, {'c', 2}, {'d', 4}, {'e', 12}, {'f', 2}, {'g', 3}, {'h', 2}, {'i', 9}, 
...
void set_rack(char rack[]) {
    for (int i = 0; i < RACK_SIZE; i++) {
        if (tile_bag[rack[i]] > 0) {
            cout << "added to rack, ";
            tile_bag[rack[i]]--;
        }
        else {
            cout << "could not add to rack - draw again!";
        }
        cout << endl << rack[i] << ' ' << std::to_string(tile_bag[rack[i]]) << endl;
    }
}
	    ...
	    switch(orientation) {
	        ...
	        case 'a':
	            if (col + word.length() < NUM_COLS && row < NUM_ROWS) {
	                ...
	                for (int i = 0; i < word.length(); i++) {
                        if (word[i] != scrabble_board[row][col+i]) {
...
void read_all_words_for_orientation(string orientation) {
    string word;
    int num_words = 0;
    cout << "Enter words " << orientation << ":\n";
    read_int_persistently(num_words);
    cout << "# words " << orientation << ": " << num_words << endl;
    int row = NUM_ROWS, col = NUM_COLS;
    while (num_words-- > 0) {
        read_int_persistently(row);
        read_int_persistently(col);
        cin >> word;
        add_word(row, col, word, orientation[0]);
    }
}
...
void set_board() {
    ...
    read_all_words_for_orientation("across");
    read_all_words_for_orientation("down");
}
